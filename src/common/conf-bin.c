...
static ssize_t __check_head(int fd)
{
	...
	if ((r = readn(fd, &bh, sizeof(bh))) != sizeof(bh)) {
		if (r == 0)
			return 0;	/* empty file ? */
		DBG_PERROR("readn bin_head %zd failed", r);
		return -1;
	}
	...
	if (magic != MAGIC_HEAD || version < CONF_VERSION) {
		DBG_PRINTF("magic(%x) or version(%x) error", magic, version);
		return -1;
	}
...
static inline ssize_t __write_head(int fd)
{
	...
	if (writen(fd, &bh, sizeof(bh)) != sizeof(bh)) {
		DBG_PERROR("writen bin_head failed");
		return -1;
	}
...
static ssize_t __check_copy(int fd, int fd_new, size_t count)
{
	...
	while (offset < count) {
		...
		if (readn(fd, buf, n) != n) {
			DBG_PERROR("read(%zu %zu %zu) from %d failed",
				   count, offset, n, fd);
			return -1;
		}

		if (fd_new >= 0 && write(fd_new, buf, n) != n) {
			DBG_PERROR("write buf %zu failed", n);
			return -1;
		}
...
static ssize_t __cmp_key(int fd, const void *key, size_t key_len)
{
	...
	while (offset < key_len) {
		...
		if (readn(fd, buf, n) != n) {
			DBG_PERROR("read key(offset = %zu, n = %zu) failed",
				   offset, n);
			return -2;
		}
			  ...
			  size_t value_len)
{
	...
	while ((r = readn(fd, &bi, sizeof(bi))) == sizeof(bi)) {
		...
		if (magic != MAGIC_ITEM) {
			DBG_PRINTF("magic(%x) dismatch, check format", magic);
			return -2;
		}
		...
		if (klen != key_len || vlen != value_len ||
			...
			if (__check_write(fd_new, &bi, sizeof(bi)) < 0 ||
				...
				DBG_PERROR("__check_write/__check_copy failed");
				return -2;
			}
			continue;
		}
...
ssize_t conf_delete_bin(const char *file)
{
	return unlink(file);
}
